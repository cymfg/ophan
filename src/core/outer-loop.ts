...
export class OuterLoop {
  ...
  constructor(options: OuterLoopOptions) {
    ...
    this.registry.register(new DevAgent());

    // Register orchestrator if enabled
    if (options.config.orchestrator?.enabled) {
  ...
  async execute(options: OuterLoopExecuteOptions = {}): Promise<OuterLoopResult> {
    ...
    // Set state on DevAgent (it needs learnings for consolidation)
    const devAgent = this.registry.get('dev-agent') as DevAgent | undefined;
    if (devAgent) {
    ...
    // Run context analysis directly via ContextLogger
    const contextLogger = new ContextLogger({ ophanDir: this.options.ophanDir });
    const contextProposals = await contextLogger.generateProposals(
      this.options.config.outerLoop.lookbackDays
    );
    if (contextProposals.length > 0) {
      this.log(`Context analysis generated ${contextProposals.length} proposal(s)`);
    }

    // Collect all proposals (limited by maxProposals)
    const allProposals = [...registryResult.proposals, ...contextProposals];
    const proposals = allProposals.slice(
      0,
    ...
    // Get consolidation stats from dev agent result
    const taskAgentResult = registryResult.agentResults.get('dev-agent');
    ...
    if (taskAgentResult?.summary) {
      // The DevAgent tracks this internally, for now just report what we have
      // In a future refactor, DevAgent could expose this more cleanly
    }
  ...
  private async generateDigest(
    ...
    const taskAgentProposals = proposals.filter((p) => p.source === 'dev-agent' || p.source === 'task-agent');
...
### Dev Agent (${taskAgentProposals.length})
...
### Context Analysis (${contextProposals.length})
